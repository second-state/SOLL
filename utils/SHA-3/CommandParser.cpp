...
template <typename F> int readFileIntoFunc(const char *fileName, F f) {
...
int hashFile(const char *fileName, const std::string &hashName, F1 &hashObj) {
  ...
  if (readFileIntoFunc(fileName,
                       [&hashObj](uint8_t *buf, unsigned int bLength) {
                         hashObj.addData(buf, 0, bLength);
                       }) == 0) {
    return 0;
  }
...
void usage() {
  puts("\n\nUsage: sha3sum [command]* file*\n"
       "\n"
       " where command is an optional parameter that can set either the "
       "algorithm, as\n"
       " there is a slight difference between the bare keccak function and the "
       "SHA-3\n"
       " variant.\n"
       "\n"
       " Algorithm \n"
       ...
       " Digest size (SHAKE)\n"
       "\n"
       " -d=number : Set the SHAKE digest size. Should be less than or equal "
       "to the hash size.\n"
       "		should be multiple of 8.\n"
       "       Only relevant for SHAKE - For SHA-3 and keccak, digest size is "
       "equal to sponge size.\n"
       "\n"
       "Any number of files can be specified. Files will be processed with the "
       "most\n"
       "recently specified options - for example:\n"
       "\n"
       "  sha3sum test.txt -a=k -w=384 test.txt -a=s -w=256 text.txt\n"
       "\n"
       "will hash \"test.txt\" three times - First with 512-bit SHA-3, then "
       "with 384-bit\n"
...
int parseDigestWidth(const char *param, const unsigned int pSize,
  ...
  if (index + 3 == pSize) {
    if (isNumeric(&param[index], pSize - index)) {
      ...
      if (sdl % 8 != 0) {
        std::cerr << "Error - param: " << param + index
                  << " is not divisible by 8.\n";
        return 0;
      }
      if (sdl > opt.hashWidth) {
        std::cerr << "Error - param: " << param + index
                  << " is greater than the hash size.\n";
        return 0;
      }
    ...
    std::cerr << "Error - param: " << param + index
              << " - expecting a three digit number.\n";
    return 0;
  }
}
