...
object "runtime" {
        code {
            function bar(x, y, z) -> return_val
            {
                let my_array := alloc(96)
                mstoren(add(my_array, mul(0, 32)), x, 32)
                mstoren(add(my_array, mul(1, 32)), y, 32)
                mstoren(add(my_array, mul(2, 32)), z, 32)
                {
                    return_val := my_array
                    leave
                }
                free(add(return_val, 96))
            }
            function avail() -> ptr
            { ptr := mload(0x00) }
            ...
            switch cloadn(0, 4)
            case 0x9309b76e {
                return(bar(calldataload(4), calldataload(36), calldataload(68)), 96)
            }
        }
    }
