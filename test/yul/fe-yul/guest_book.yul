...
object "runtime" {
        code {
            function sign(book_msg)
            {
                let ptr := avail()
                mcopy(book_msg, dualkeccak256(0, caller()), 100)
                log1(book_msg, 128, 0xf95318ba442251854c1277ed370e2adb2cd6dc2156bdffdd75dc5d798b31ab0f)
                free(ptr)
            }
            function get_msg(addr) -> return_val
            {
                {
                    return_val := scopy(dualkeccak256(0, addr), 100)
                    leave
                }
                free(add(return_val, 100))
            }
            function avail() -> ptr
            { ptr := mload(0x00) }
            ...
            switch cloadn(0, 4)
            case 0x7f8fe731 { sign(ccopy(4, 100)) }
            case 0xd60bed18 {
                return(get_msg(calldataload(4)), 128)
            }
        }
    }
